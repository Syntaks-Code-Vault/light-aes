#include "ecb.h"

#define GF  0b100011011

const byte S_BOX[] = {  , };

const byte INV_S_BOX[] = {  , };

byte* expand_key(byte* key, byte key_length) {
    byte Nk = key_length / 4;
    byte Nr = 0;
    switch (key_length) {
        case KEY_SIZE_128:
            Nr = 10;
            break;
        case KEY_SIZE_192:
            Nr = 12;
            break;
        case KEY_SIZE_256:
            Nr = 14;
            break;
    }

    byte Nw = (1 + Nr) * 4;
    byte* round_keys = (byte*) malloc(Nw * 4);

    for (byte b = 0 ; b < key_length ; b++)
        round_keys[b] = key[b];
    
    byte wi = key_length / 4;
    byte rcon = 0x01;

    for ( ; wi < Nw ; wi++) {
        if (wi % Nk == 0) {
            round_keys[4 * wi] = S_BOX[round_keys[4 * (wi - 1) + 1]] ^ rcon ^ round_keys[4 * (wi - Nk)];
            round_keys[4 * wi + 1] = S_BOX[round_keys[4 * (wi - 1) + 2]] ^ round_keys[4 * (wi - Nk) + 1];
            round_keys[4 * wi + 2] = S_BOX[round_keys[4 * (wi - 1) + 3]] ^ round_keys[4 * (wi - Nk) + 2];
            round_keys[4 * wi + 3] = S_BOX[round_keys[4 * (wi - 1) ]] ^ round_keys[4 * (wi - Nk) + 3];

            rcon <<= 1;
            if (rcon == 0)
                rcon = 0x1b;
        } else {
            if (key_length == KEY_SIZE_256 && (wi - 4) % Nk == 0)
                for (byte i = 0 ; i < 4 ; i++)
                    round_keys[4 * wi + i] = S_BOX[round_keys[4 * (wi - 1) + i]] ^ round_keys[4 * (wi - Nk) + i];
            else
                for (byte i = 0 ; i < 4 ; i++)
                    round_keys[4 * wi + i] = round_keys[4 * (wi - 1) + i] ^ round_keys[4 * (wi - Nk) + i];
            }
    }

    return round_keys;
}

aes_ecb* create_aes_ecb_instance(const byte* key, byte key_length) {
    aes_ecb* instance = (aes_ecb*) malloc(sizeof(aes_ecb));

    if (key_length == KEY_SIZE_128 || key_length == KEY_SIZE_192 || key_length == KEY_SIZE_256) {
        instance -> key_length = key_length;

        instance -> key = (byte*) malloc(key_length);
        do {
            instance -> key[key_length] = key[key_length];
        } while (key_length-- > 0);
    }

    return instance;
}

byte mul_GF(byte val, byte m) {
    unsigned short r = 0;
    byte k = 0;

    while (m != 0) {
        if (m & 0x01)
            r ^= (val << k);

        m >>= 1;
        k++;
    }
    
    k = 8;
    while (r & 0xFF00) {
        if (r & (1 << k))
            r ^= (GF << (k - 8));

        k++;
    }

    return r;
}

void sub_bytes(byte* state) {
    for (byte i = 0 ; i < 16 ; i++)
        state[i] = S_BOX[state[i]];
}

void shift_rows(byte* state) {
    byte t;

    t = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = t;

    t = state[2];
    state[2] = state[10];
    state[10] = t;
    t = state[6];
    state[6] = state[14];
    state[14] = t;

    t = state[3];
    state[3] = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = t;
}

void mix_columns(byte* state) {
    byte a, b, c, d;

    for (byte i = 0 ; i < 4 ; i++) {
        a = state[4 * i + 0];
        b = state[4 * i + 1];
        c = state[4 * i + 2];
        d = state[4 * i + 3];
        state[4 * i + 0] = mul_GF(a, 0x2) ^ mul_GF(b, 0x3) ^ c ^ d;
        state[4 * i + 1] = mul_GF(b, 0x2) ^ mul_GF(c, 0x3) ^ d ^ a;
        state[4 * i + 2] = mul_GF(c, 0x2) ^ mul_GF(d, 0x3) ^ a ^ b;
        state[4 * i + 3] = mul_GF(d, 0x2) ^ mul_GF(a, 0x3) ^ b ^ c;
    }
}

void add_round_key(byte* state, byte* round_key) {
    for (byte i = 0 ; i < 16 ; i++)
        state[i] ^= round_key[i];
}

void inv_mix_columns(byte* state) {
    byte a, b, c, d;

    for (byte i = 0 ; i < 4 ; i++) {
        a = state[4 * i + 0];
        b = state[4 * i + 1];
        c = state[4 * i + 2];
        d = state[4 * i + 3];
        state[4 * i + 0] = mul_GF(a, 0xE) ^ mul_GF(b, 0xB) ^ mul_GF(c, 0xD) ^ mul_GF(d, 0x9);
        state[4 * i + 1] = mul_GF(b, 0xE) ^ mul_GF(c, 0xB) ^ mul_GF(d, 0xD) ^ mul_GF(a, 0x9);
        state[4 * i + 2] = mul_GF(c, 0xE) ^ mul_GF(d, 0xB) ^ mul_GF(a, 0xD) ^ mul_GF(b, 0x9);
        state[4 * i + 3] = mul_GF(d, 0xE) ^ mul_GF(a, 0xB) ^ mul_GF(b, 0xD) ^ mul_GF(c, 0x9);
    }
}

void inv_shift_rows(byte* state) {
    byte t;

    t = state[13];
    state[13] = state[9];
    state[9] = state[5];
    state[5] = state[1];
    state[1] = t;

    t = state[2];
    state[2] = state[10];
    state[10] = t;
    t = state[6];
    state[6] = state[14];
    state[14] = t;

    t = state[7];
    state[7] = state[11];
    state[11] = state[15];
    state[15] = state[3];
    state[3] = t;
}

void inv_sub_bytes(byte* state) {
    for (byte i = 0 ; i < 16 ; i++)
        state[i] = INV_S_BOX[state[i]];
}

void encrypt_ecb(aes_ecb* instance, byte* plaintext) {
    byte Nr = 0;
    switch (instance -> key_length) {
        case KEY_SIZE_128:
            Nr = 10;
            break;
        case KEY_SIZE_192:
            Nr = 12;
            break;
        case KEY_SIZE_256:
            Nr = 14;
            break;
    }

    byte Nk = instance -> key_length / 4;

    byte* round_keys = expand_key(instance -> key, instance -> key_length);
    
    add_round_key(plaintext, round_keys);

    byte r;
    for (r = 1 ; r < Nr ; r++) {
        sub_bytes(plaintext);
        shift_rows(plaintext);
        mix_columns(plaintext);
        add_round_key(plaintext, round_keys + (16 * r));
    }

    sub_bytes(plaintext);
    shift_rows(plaintext);
    add_round_key(plaintext, round_keys + (16 * r));
}

void decrypt_ecb(aes_ecb* instance, byte* ciphertext) {
    byte* s = ciphertext;
    
    byte Nr = 0;
    switch (instance -> key_length) {
        case KEY_SIZE_128:
            Nr = 10;
            break;
        case KEY_SIZE_192:
            Nr = 12;
            break;
        case KEY_SIZE_256:
            Nr = 14;
            break;
    }

    byte Nk = instance -> key_length / 4;

    byte* round_keys = expand_key(instance -> key, instance -> key_length);
    
    add_round_key(s, round_keys + (16 * Nr));
    
    byte r;
    for (r = Nr - 1 ; r > 0 ; r--) {
        inv_shift_rows(s);
        inv_sub_bytes(s);
        add_round_key(s, round_keys + (16 * r));
        inv_mix_columns(s);
    }

    inv_shift_rows(s);
    inv_sub_bytes(s);
    add_round_key(s, round_keys);
}